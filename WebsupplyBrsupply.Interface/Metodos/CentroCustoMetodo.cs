            ...
            try
            {
                ...
                if (DadosUnidade.Rows.Count > 0)
                {
                    for (int i = 0; i < DadosUnidade.Rows.Count; i++)
                    {
                        ...
                        do
                        {
                            ...
                            if (retornoAPI.Count > 0)
                            {
                                ...
                                for (int j = 0; j < retornoAPI.Count; j++)
                                {
                                    ...
                                    {
                                        CentroCusto = linhaRetorno["CTT_CUSTOS"].ToString().Trim(),
                                        Descricao = linhaRetorno["CTT_DESC01"].ToString().Trim(),
                                        Status = linhaRetorno["CTT_MSBLQL"].ToString().Trim()
                                    };
                                    ...
                                    arrParam2.Add(new Parametro("@cStatus", centroCusto.Status == "" ? null : centroCusto.Status.ToString(), SqlDbType.Char, 1, ParameterDirection.Input));

                                    ArrayList arrOut2 = new ArrayList();

                                    conn.ExecuteStoredProcedure(new StoredProcedure("SP_HHEMO_WS_CCUSTO_INSUPD", arrParam2), ref arrOut2);
                                }
                        ...
                        if (totalRegistros > 0)
                        {
                            strMensagemInterna = $"{totalRegistros} Centro(s) de Custo cadastrados/atualizados com sucesso para a Empresa {DadosUnidade.Rows[i]["descricao"]}";
